...
using GTDoro.Web.ViewModels.Identity;
...
using SignInStatus = GTDoro.Web.Models.Identity.SignInStatus;
...
namespace GTDoro.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        // GET: /Account/LightLogin
        [AllowAnonymous]
        public ActionResult LightLogin(string returnUrl)
        {
            ViewBag.ReturnUrl = returnUrl;
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FullName = model.Name, CreationDate = DateTime.UtcNow, TimeZoneId = model.TimeZoneId };
                ...
                if (result.Succeeded)
                {
                    ...
                    string regMessageSubject = db.GetParameterValueByCode(ParameterType.RegMessageSubject);
                    if(string.IsNullOrWhiteSpace(regMessageSubject))
                    {
                        regMessageSubject = Settings.DEFAULT_REG_MSG_SUBJECT;
                    }
                    string regMessageContent = db.GetParameterValueByCode(ParameterType.RegMessageContent);
                    if (string.IsNullOrWhiteSpace(regMessageContent))
                    {
                        regMessageContent = Settings.DEFAULT_REG_MSG_CONTENT;
                    }

                    await UserManager.SendEmailAsync(user.Id, regMessageSubject, regMessageContent + " <a href=\"" + callbackUrl + "\">Confirmation link</a>");
                    
                    //HERE SEND EMAIL WITH THE LINK (callbackUrl)
                    //Please change this code to register an email service in IdentityConfig to send an email.

                    //return RedirectToAction("DisplayEmail", "Home", new { RegistrationLink = callbackUrl });
                    return RedirectToAction("ThankYouForRegistering", "Home");
                }
        ...
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                CreateDefaultItems(userId);
                return RedirectToAction("ConfirmEmail", "Home", new { IsEmailConfirmation = "ok" });
            }
            AddErrors(result);
            return View();
        }
